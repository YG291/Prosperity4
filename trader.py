...
class Trader:
    ...
    def _with_market_orders(self, sorted_list: list[tuple], storage, product: str):
        for price, quantity in sorted_list:
            ...
            q, val = (storage[product]["sort"][0],
                      storage[product]["sort"][1])  # total order volume, mean of all order offers
            new_q = q + quantity
            if new_q > 0:
    ...
    def run(self, state: TradingState):
        ...
        if state.traderData:
            ...
            storage = {"ASH_COATED_OSMIUM": {"historical": {}, "sort": [0, 0], "buy": [0, 0], "sell": [0, 0]},
                       "INTARIAN_PEPPER_ROOT": {"historical": {}, "sort": [0, 0], "buy": [0, 0], "sell": [0, 0]},
